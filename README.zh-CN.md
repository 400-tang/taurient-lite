...
**线上简报：** https://taurient-lite-backend.onrender.com
...
- **独立的日历标签页**：接下来的事件摆在滚动周网格上，哪几天扎堆一眼可见；
  ...
  不会为了视觉整齐编一个假日期
- **板块热力**：简报首页的头一块。十一个行业各一张矩形树图，块的大小
...
4. 取数并渲染：`apply_momentum.py`、`fetch_sectors.py`，然后 `render.py`
...
- **页面上的数字也是活的**：`/` 渲染时会就地把板块热力换成现场抓的实时数据
  （缓存 5 分钟），盘中打开就是盘中的数字。新闻仍然是
  每天一次的批处理产物——那部分需要模型判断，但数字不该跟着一起等到明天。
  取不到就退回简报里的存档，绝不让页面变成 502；翻看历史日期时一律用存档，
  给旧简报配今天的行情会让两者被当成同一天的事实
